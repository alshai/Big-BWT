 ...
 * newscan.cpp
 * 
 * parsing algorithm for the bwt constrruction for repetitive sequences based 
 * on prefix free parsing
 * 
 * Usage:
 *   newscan.x wsize modulus file
 * 
 * Accept any kind of file that does not contain the chars 0, 1, 2 which are used 
 * internally. If input file is gzipped use cnewscan.x which automatcally
 ...
 *     T = (02)file_content(02)^wsize
 * in a dictionary of words D and a parsing P of T in terms of 
 * the dictionary words. Note the words in the parsing overalp by wsize parameters.
 * Let d denote the number of words in D and p the number of frases in the parsing P
 * 
 * newscan outputs the following files:
 * 
 * file.dict
 * containing the dictionary words in lexicogaphic order with a 01 at the end of
 * each word and a 00 at the end of the file. Size: |D| + d + 1 where
 * |D| is the sum of the word lengths
 * 
 * file.occ
 * the number of occurrences of each word in lex. order in 32 bit uint. 
 * Size 4d
 * 
 * file.parse
 * containing the parse P with each word identified with its 1-based lexicographic 
 * rank (ie its position in D). Size 4p
 * 
 * file.last 
 ...
 * 
 * 
 * The output of newscan should be processed by bwtparse, that, invoked as
 * 
 *    bwtparse file
 * 
 * computes the BWT of file.parse and produces file.ilist of size 4p+4
 * contaning for each dictionary word the list of BWT positions where that
 * word appeat (ie i\in ilist(w) <=> BWT[i]=w). There is also an entry
 * for the EOF word which is not in the dictionary but is assumed to be the 
 * smallest word.  In addition, bwtparse permutes file.last according to
 ...
 * if SA[i]==1, BWT[i]=P[0] and file.bwlast[i] is taken from P[n-1], the last word 
 * in the parsing).  
 * 
 * 
 * 
...
// save current word in freq and update it leaving only the last minsize chars 
// which is the overlap with next word  
void save_update_word(string& w, unsigned int minsize,map<uint64_t,word_stats>&  freq, FILE *tmp_parse_file, FILE *last)
{
  ...
  if(freq.find(hash)==freq.end()) {
      ...
      if(freq[hash].str != w)
        die("Hash collision for strings: " + freq[hash].str + " vs " + w);
  }
...
void process_file(int w, int p, string fnam, KR_window& krw, map<uint64_t,word_stats>& wordFreq, FILE *tmp_parse_file)
{
  ...
  // open file with last char of each word
...
void writeDictOcc(map<uint64_t,word_stats> &wfreq, vector<const string *> &sortedDict, string fname)
{
  ...
  for(auto x: sortedDict) {
    ...
    if(wf.rank!=0) die("totally unexpect hash collision");
    wf.rank = wrank++;
  }
